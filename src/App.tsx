...
export default function App() {
  ...
  const handleLogin = (adminName: 'Akash' | 'Alojyoti' | 'Sumanta') => {
    ...
    localStorage.setItem('admin_login_name', adminName);
  };
      ...
      <header className="sticky top-0 z-30 glass-panel border-b border-slate-200/80 dark:border-slate-800/40 no-print">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div id="header-control-cluster" className="flex items-center gap-3">
            ...
            {/* Active Admin Pill */}
            <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 p-1.5 rounded-xl text-xs font-semibold text-slate-800 dark:text-slate-355 text-slate-700 dark:text-slate-300">
              <div className="w-5 h-5 bg-indigo-600 text-white flex items-center justify-center rounded-lg font-bold text-[10px]">
                {currentAdminName[0]}
              </div>
              <span className="hidden xs:inline-block max-w-[80px] truncate font-bold font-sans">{currentAdminName}</span>
            </div>

            {/* Logout Button */}
            <button
              onClick={handleLogout}
              className="p-2 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-930 text-rose-500 hover:text-rose-600 dark:text-rose-400 hover:bg-rose-500/5 transition-all cursor-pointer flex items-center gap-1.5 shrink-0"
              title="Sign Out of Admin Console"
            >
              <LogOut className="h-4.5 w-4.5" />
              <span className="hidden md:inline-block text-[11px] font-bold italic font-mono">Exit</span>
            </button>
          </div>
